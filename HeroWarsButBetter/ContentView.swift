...
struct ContentView: View {
    ...
    @State private var navigateToLost = false
    let scene = GameScene()
    ...
    var body: some View {
        ...
        .onChange(of: viewModel.battle.state) { newState in
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                if newState == .won(team: "Player") {
                    navigateToWin = true
                } else if newState == .won(team: "Enemy") {
                    navigateToLost = true
                }
            }
        }
        .fullScreenCover(isPresented: $navigateToWin, content: {
            WinView()
        })
        .fullScreenCover(isPresented: $navigateToLost, content: {
            LostView()
        })
    }
}
